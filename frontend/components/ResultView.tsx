...
export const ResultView: React.FC<ResultViewProps> = ({
  ...
  // Generate a safe filename using current date
  const today = new Date().toISOString().slice(0, 10);
  const safeFilename = `captioned-video-${today}.mp4`;
    ...
    <div className="space-y-8 w-full max-w-5xl mx-auto">
      ...
      <div className="p-4 sm:p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 flex items-center justify-between flex-wrap gap-4 shadow-lg shadow-emerald-950/30">
        <div className="flex items-center gap-3.5">
          ...
          <div>
            <h3 className="font-bold text-white text-base sm:text-lg">Video Ready!</h3>
            <p className="text-xs text-emerald-400/90">
              Captions rendered and burned successfully. Preview or download below.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {onBackToEditor && (
            <button
              ...
              className="px-4 py-2 rounded-xl bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-300 hover:text-indigo-200 text-xs font-semibold flex items-center gap-2 transition-colors border border-indigo-500/30"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-7 space-y-3">
          <div className="relative rounded-2xl overflow-hidden bg-black border border-slate-800 shadow-2xl min-h-[300px] max-h-[520px] flex items-center justify-center group ring-1 ring-slate-800 p-2">
            <video
              src={previewUrl}
              controls
          ...
          <div className="flex items-center justify-between text-xs text-slate-400 px-2">
            <span className="flex items-center gap-1.5 text-indigo-400 font-medium">
              <Play className="w-3.5 h-3.5" /> 1080p HD • Subtitles Burned In
            </span>
            <span>
              Style: <strong className="text-slate-200 capitalize">{jobStatus.style || 'Classic'}</strong>
            </span>
          </div>
        </div>
          ...
          <div className="glass-panel p-6 rounded-2xl border border-slate-800 space-y-4">
            ...
            <div className="grid grid-cols-3 gap-2 pt-2 border-t border-slate-800">
              ...
              <a
                ...
                download
                className="py-2.5 px-2 rounded-xl bg-slate-900/80 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-700/80 text-xs font-medium flex items-center justify-center gap-1.5 transition-colors"
                title="Download ASS subtitle file"
              >
              ...
              <a
                ...
                download
                className="py-2.5 px-2 rounded-xl bg-slate-900/80 hover:bg-slate-800 text-slate-300 hover:text-white border border-slate-700/80 text-xs font-medium flex items-center justify-center gap-1.5 transition-colors"
                title="Download JSON timestamps"
              >
            ...
            {onBackToEditor && (
              <button
                onClick={onBackToEditor}
                className="w-full py-3 px-4 rounded-xl bg-indigo-500/15 hover:bg-indigo-500/25 text-indigo-300 border border-indigo-500/30 text-xs font-bold flex items-center justify-center gap-2 transition-colors"
              >
                <Edit2 className="w-3.5 h-3.5" />
            ...
            <div className="pt-3 border-t border-slate-800/80">
              {confirmDelete ? (
                ...
                >
                  ...
                  <span>Delete Video from Server</span>
                </button>
              )}
            </div>
          </div>
